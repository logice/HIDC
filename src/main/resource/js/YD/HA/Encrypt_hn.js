...
(function($) {
	...
	$.jCryption.getKeys = function(e,n,maxdigits,callback) {
		var base = this;
		base.getKeys = function() {
			keys = new base.jCryptionKeyPair(e,n,maxdigits);
...
function login_wsyyt() {
	var broUUid = Math.uuid();
    $.jCryption.getKeys("./rsaKey?broUUid="+broUUid+"&random="+Math.random()+"&jsoncallback=?",function(receivedKeys) {
    	var keys = receivedKeys;
        $.jCryption.encrypt($("input[name='mobileno']").val(), keys, function(name) {
        	$('#IDToken1').val(name);
        	var psType=$("input[name='IDToken2']").val();
        	var psw = $("input[name='FwPassword']").val();
        	if(psType=='2'){
        	}else {
        		psw = $("input[name='SMSPassword']").val();
        	}
        	$.jCryption.encrypt(psw, keys, function(pssword) {
        		$('#IDToken3').val(pssword);
        		$('#IDToken4').val($("input[name='rand']").val());
        		$('#IDToken5').val(broUUid);
        		$('#login_form').submit();
        	});
        });	
    });
...
function loginNameEncrypt(e,n,maxdigits,mobile) {
	var broUUid = Math.uuid();
    $.jCryption.getKeys(e,n,maxdigits,function(receivedKeys) {
    	var keys = receivedKeys;
        $.jCryption.encrypt(mobile, keys, function(name) {
			return name;
        });	
    });
}